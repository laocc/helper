 ...
 *
 * 双向对称加密：
 * 4：base_en    对base64结果重排，支持中文；    反解：base_de         用 string_disc_rand 生成转换因子
 * 5：str_en     用进制换算加密重排，支持中文；   反解：str_de          转换因子在 str_conversion 中手工添加
 * 6：int_en     对整数加密                    反解：int_de          转换因子在 int_conversion 中手工添加
 *
 * 双向不对称加密：
 *
 */
class Password
{
     ...
     * @return string
     */
    public static function md3(string $str, bool $double = false): string
    {
...
//        echo "原生MD5={$md}[{$len}]='" . $md[$len] . "'的asc(" . $md[$len] . ")=" . ord($md[$len]) . "，取5模={$i}字典】";
    ...
    public static function md3_check($string, $password): bool
    {
     ...
     */
    public static function md3_disc_rand(int $num = 10)
    {
        for ($z = 0; $z < $num; $z++) {
            $arr = array();
     ...
     * @return null|string
     */
    public static function md4($password): ?string
    {
     ...
     */
    public static function md4_check($string, $password): bool
    {
     ...
     * @return bool|string
     */
    public static function hash(string $string, $type = PASSWORD_DEFAULT)
    {
     ...
     * @return bool
     */
    public static function hash_check(string $string, string $hash): bool
    {
     ...
     * @return string
     */
    public static function base_en(string $String, int $expiration = 0): string
    {
        //创建时间，内容，有效时间
        $str = base64_encode(time() . self::$RANDOM . $String . self::$RANDOM . $expiration . self::$RANDOM . self::s_rand());
        $RS = str_split($str);
        $RS = array_reverse($RS);//倒排
        $arr = self::base_disc(count($RS));//使用密码库因子
        $new = array();
        foreach ($RS as &$rs) $new[] = $arr[$rs] ?? $rs;
     ...
     * @return string
     */
    private static function str_conversion(string $od, int $ai = 0, bool $en = true, int $Hex = 36, int $pdLen = 4): string
    {
        $arr = defined('_str_factor') ? _str_factor : [[4, 7, 1, 9, 3, 5, 2, 6, 0, 8], [6, 3, 8, 1, 0, 9, 2, 7, 5, 4]];
        if ($en) {//加密
            $arr = $arr[$ai];//取一个库表
            ...
            return sprintf("%0{$pdLen}s", $str);//转为16进制，并返回5位长，不够的前边补0
        } else {//解密
     ...
     * @return int
     */
    public static function int_en(int $int): int
    {
        $ordS = array();//换取ASC值
     ...
     * @return int
     */
    public static function int_de(int $int): int
    {
        $eS = str_split($int, 4);
        $ascS = array();
    ...
    private static function int_conversion($str, $ai = 0, $en = true): string
    {
        //转换因子，将0-9打乱即可
        $arr = defined('_int_factor') ? _int_factor : [[6, 8, 4, 1, 9, 0, 5, 2, 7, 3], [2, 8, 7, 5, 9, 3, 1, 0, 4, 6]];
        if ($en) {//加密
            $arr = $arr[$ai];//取一个库表
            for ($i = 0; $i < strlen($str); $i++) $str[$i] = $arr[$str[$i]];
            $str = base_convert($str, 10, 9);//进制转换，转为9进制
            return sprintf("%04s", $str);
        } else {//解密
            $arr = array_flip($arr[$ai]);//转码库键值对换
     ...
     * //****************************************RSA********************************************************************
     */
    public static function create_ssl($len = 1024): ?array
    {
     ...
     * @return string 密文
     */
    public static function ssl_encode($data, $pubKey = null): string
    {
        $len = 117;
        if (is_array($data)) {
            ksort($data);
...
//        $pubKey = self::replaceKey('public', $pubKey);
     ...
     * 解密
     * @param string $data
     * @param string|null $priKey
     * @return string
     */
    public static function ssl_decode(string $data, string $priKey = null): string
    {
        $ret = '';
...
//            $priKey = self::replaceKey('private', $priKey);
    ...
    /**
     * 针对公钥和私钥进行修改
     * @param string $strKey 原字符串
     * @param string $type PRIVATE: 私钥，PUBLIC：公钥
     * @return string
     */
    private static function replaceKey(string $type, string $strKey): string
    {
    ...
    private static function ssl($type): string
    {
        $key = array();
        $key['public'] = <<<PUB
-----BEGIN PUBLIC KEY-----
